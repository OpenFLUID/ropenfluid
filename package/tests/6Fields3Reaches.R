...
checkEqualsNumeric(
    as.numeric(
        OpenFLUID.getSimulatorParam(ofdata, "tests.funcA", "pA1")
    ), 0.000005
)
checkEqualsNumeric(
    as.numeric(
        OpenFLUID.getSimulatorParam(ofdata, "tests.funcC", "pC2")
    ), 0.045
)
checkEquals(
    OpenFLUID.getSimulatorParam(ofdata, "tests.funcC", "fakeparam"), ""
)
checkEquals(
    OpenFLUID.getSimulatorParam(ofdata, "tests.fakefunc", "fakeparam"), ""
)
...
dfSimParams <- OpenFLUID.getSimulatorParams(
    ofdata, "tests.funcC", c("pC2", "pC3")
)
checkEqualsNumeric(as.numeric(dfSimParams["tests.funcC", "pC2"]), 0.045)
dfSimParamsOneCol <- OpenFLUID.getSimulatorParams(
    ofdata, "tests.funcC", c("pC2")
)
checkEqualsNumeric(as.numeric(dfSimParamsOneCol["tests.funcC", "pC2"]), 0.045)

checkEqualsNumeric(
    as.numeric(
        OpenFLUID.getGeneratorParam(ofdata, "SU", "tests.fixed", "fixedvalue")
    ), 12.7
)
checkEqualsNumeric(
    as.numeric(
        OpenFLUID.getGeneratorParam(ofdata, "RS", "tests.random", "min")
    ), 20.53
)
checkEqualsNumeric(
    as.numeric(
        OpenFLUID.getGeneratorParam(ofdata, "RS", "tests.random", "max")
    ), 50
)
...
dfGenParams <- OpenFLUID.getGeneratorParams(
    ofdata, "RS", "tests.random", c("min", "max")
)
checkEqualsNumeric(as.numeric(dfGenParams["tests.random", "min"]), 20.53)
dfGenParamsOneCol <- OpenFLUID.getGeneratorParams(
    ofdata, "RS", "tests.random", c("min")
)
...
checkEqualsNumeric(
    as.numeric(
        OpenFLUID.getObserverParam(ofdata, "tests.obsA", "pA1")
    ), 10
)
checkEquals(
    OpenFLUID.getObserverParam(ofdata, "tests.obsA", "pA2"), "valA2"
)
checkEquals(
    OpenFLUID.getObserverParam(ofdata, "tests.obsA", "fakeparam"), ""
)
checkEquals(
    OpenFLUID.getObserverParam(ofdata, "tests.fakeobs", "fakeparam"), ""
)
...
dfObsParams <- OpenFLUID.getObserverParams(
    ofdata, "tests.obsA", c("pA2", "pA1")
)
checkEquals(dfObsParams["tests.obsA", "pA2"], "valA2")
dfObsParamsOneCol <- OpenFLUID.getObserverParams(
    ofdata, "tests.obsA", c("pA2")
)
checkEquals(dfObsParamsOneCol["tests.obsA", "pA2"], "valA2")

checkEqualsNumeric(
    as.numeric(
        OpenFLUID.getAttribute(ofdata, "SU", 1, "area")
    ), 1216.29
)
checkEqualsNumeric(
    as.numeric(
        OpenFLUID.getAttribute(ofdata, "SU", 5, "area")
    ), 3024.27
)
checkEqualsNumeric(
    as.numeric(
        OpenFLUID.getAttribute(ofdata, "SU", 3, "slope")
    ), 0.02
)
checkEqualsNumeric(
    as.numeric(
        OpenFLUID.getAttribute(ofdata, "RS", 2, "length")
    ), 170
)
checkEqualsNumeric(
    OpenFLUID.getAttribute(ofdata, "RS", 18, "length"), ""
)
checkEqualsNumeric(
    OpenFLUID.getAttribute(ofdata, "RS", 1, "fakedata"), ""
)
...
dfVal <- OpenFLUID.getAttributes(
    ofdata, "SU", c(1, 5), c("area", "slope")
)
checkEqualsNumeric(
    dfVal["SU#1", "area"], OpenFLUID.getAttribute(ofdata, "SU", 1, "area")
)
checkEqualsNumeric(
    dfVal["SU#5", "area"], OpenFLUID.getAttribute(ofdata, "SU", 5, "area")
)
checkEqualsNumeric(
    dfVal["SU#1", "slope"], OpenFLUID.getAttribute(ofdata, "SU", 1, "slope")
)
checkEqualsNumeric(
    dfVal["SU#5", "slope"], OpenFLUID.getAttribute(ofdata, "SU", 5, "slope")
)

dfValWithID <- OpenFLUID.getAttributes(
    ofdata, "SU", c(1, 5), c("area", "slope"), unitidsAsRownames = FALSE
)
checkEqualsNumeric(
    dfVal[1, "area"], OpenFLUID.getAttribute(ofdata, "SU", 1, "area")
)
checkEqualsNumeric(
    dfVal[2, "area"], OpenFLUID.getAttribute(ofdata, "SU", 5, "area")
)
checkEqualsNumeric(
    dfVal[1, "slope"], OpenFLUID.getAttribute(ofdata, "SU", 1, "slope")
)
checkEqualsNumeric(
    dfVal[2, "slope"], OpenFLUID.getAttribute(ofdata, "SU", 5, "slope")
)

checkEquals(rownames(dfVal), dfValWithID$unitid)

# check get attributes in batch mode for one row
dfValOneRow <- OpenFLUID.getAttributes(
    ofdata, "SU", c(2), c("area", "slope")
)
checkEqualsNumeric(
    dfValOneRow["SU#2", "area"],
    OpenFLUID.getAttribute(ofdata, "SU", 2, "area")
)
checkEqualsNumeric(
    dfValOneRow["SU#2", "slope"],
    OpenFLUID.getAttribute(ofdata, "SU", 2, "slope")
)

# check get attributes in batch mode for one row
dfValOneRowOneCol <- OpenFLUID.getAttributes(
    ofdata, "SU", c(2), c("area")
)
checkEqualsNumeric(
    dfValOneRow["SU#2", "area"],
    OpenFLUID.getAttribute(ofdata, "SU", 2, "area")
...
dfValPlusOne <- dfVal
dfValPlusOne$area <- as.character(
    as.numeric(dfValPlusOne$area) + c(1, 1)
) # add 1 to column "area"
dfValPlusOne$slope <- as.character(
    as.numeric(dfValPlusOne$slope) + c(1, 1)
) # add 1 to column "slope"
OpenFLUID.setAttributes(ofdata, "SU", dfValPlusOne)

checkEquals(
    as.numeric(dfVal[1, "area"]),
    as.numeric(OpenFLUID.getAttribute(ofdata, "SU", 1, "area")) - 1
)
checkEquals(
    as.numeric(dfVal[2, "area"]),
    as.numeric(OpenFLUID.getAttribute(ofdata, "SU", 5, "area")) - 1
)
checkEquals(
    as.numeric(dfVal[1, "slope"]),
    as.numeric(OpenFLUID.getAttribute(ofdata, "SU", 1, "slope")) - 1
)
checkEquals(
    as.numeric(dfVal[2, "slope"]),
    as.numeric(OpenFLUID.getAttribute(ofdata, "SU", 5, "slope")) - 1
)
...
checkEquals(
    OpenFLUID.getSimulatorParam(ofdata, "tests.funcB", "pB1"), "codetoto"
)
checkEqualsNumeric(
    as.numeric(
        OpenFLUID.getGeneratorParam(ofdata, "RS", "tests.random", "min")
...
checkEqualsNumeric(
    as.numeric(OpenFLUID.getModelGlobalParam(ofdata, "gparam1")), 3.1
)
checkEqualsNumeric(
    as.numeric(OpenFLUID.getModelGlobalParam(ofdata, "gparam2")), 18
)
checkEqualsNumeric(
    as.numeric(OpenFLUID.getObserverParam(ofdata, "tests.obsA", "pA1")), 0.5
)
checkEquals(
    OpenFLUID.getObserverParam(ofdata, "tests.obsB", "format"), "format1"
)
checkEqualsNumeric(
    as.numeric(OpenFLUID.getAttribute(ofdata, "SU", 1, "coeff")), 1.5
)
checkEqualsNumeric(
    as.numeric(OpenFLUID.getAttribute(ofdata, "SU", 2, "coeff")), 1.5
)
checkEqualsNumeric(
    as.numeric(OpenFLUID.getAttribute(ofdata, "SU", 3, "coeff")), 37.1
)
checkEqualsNumeric(
    as.numeric(OpenFLUID.getAttribute(ofdata, "SU", 5, "coeff")), 1.5
)
...
OpenFLUID.setSimulatorParams(
    ofdata,
    "tests.funcB",
    data.frame("pB1" = "codetiti", "pB2" = 0.1, stringsAsFactors = FALSE)
)
OpenFLUID.setGeneratorParams(
    ofdata,
    "RS",
    "tests.random",
    data.frame("min" = -100, "max" = 0, stringsAsFactors = FALSE)
)
OpenFLUID.setModelGlobalParams(
    ofdata,
    data.frame("gparam1" = 3.0, "gparam2" = 28.0, stringsAsFactors = FALSE)
)
OpenFLUID.setObserverParams(
    ofdata,
    "tests.obsB",
    data.frame("format" = "format2", "precision" = 12, stringsAsFactors = FALSE)
)

checkEquals(
    OpenFLUID.getSimulatorParam(ofdata, "tests.funcB", "pB1"), "codetiti"
)
checkEqualsNumeric(
    as.numeric(OpenFLUID.getSimulatorParam(ofdata, "tests.funcB", "pB2")), 0.1
)
checkEqualsNumeric(
    as.numeric(
        OpenFLUID.getGeneratorParam(ofdata, "RS", "tests.random", "min")
...
checkEqualsNumeric(
    as.numeric(OpenFLUID.getModelGlobalParam(ofdata, "gparam1")), 3.0
)
checkEqualsNumeric(
    as.numeric(OpenFLUID.getModelGlobalParam(ofdata, "gparam2")), 28
)
checkEquals(
    OpenFLUID.getObserverParam(ofdata, "tests.obsB", "format"), "format2"
)
checkEqualsNumeric(
    as.numeric(
        OpenFLUID.getObserverParam(ofdata, "tests.obsB", "precision")
    ), 12
)
